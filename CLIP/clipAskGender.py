...
def ask_clip(image_path, model, processor, device, activity):
    """
    Uses a CLIP model to classify an image as 'male' or 'female' via zero-shot inference,
    ...
    gender_label = "male" if label_id == 0 else "female"

    return gender_label, prob_male, prob_female
...
def process_frames_with_clip(
    ...
    each frame as 'male' or 'female' (with the activity in the prompt),
    and writes to two TSV files:
    
    1. framewise_tsv (frame-level):
       [video_name, frame_file, gender_label, prob_male, prob_female]
    
    2. summary_tsv (video-level):
       [video_name, male_count, female_count, final]

    'final' is decided via majority vote (male_count vs female_count).
    """
    random.seed(seed)
    ...
    with open(summary_tsv, mode="w", newline="", encoding="utf-8") as f_summary, \
        ...
        frame_writer.writerow(["video_name", "frame_file", "gender_label", "prob_male", "prob_female"])
        ...
        for video_name in sorted(os.listdir(frames_root)):
            ...
            for frame_file in frame_files:
                ...
                gender_label, prob_male, prob_female = ask_clip(
            ...
            print(f"[INFO] {video_name} (Activity: {activity}): male={male_count}, female={female_count}, final={final_label}")
...
if __name__ == "__main__":
    ...
    2. This script loads a CLIP model and processor,
       ...
       but for 'male' vs 'female' detection.
    """
