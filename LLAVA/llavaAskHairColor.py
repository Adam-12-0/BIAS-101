...
def ask_llava(image_path, model, tokenizer, image_processor, device, activity):
    """
    Uses the LLaVA model to classify an image based on hair color.
    """
    ...
    # 2. Define the question for hair color detection
    conv_template = "qwen_1_5"
    question = (
        DEFAULT_IMAGE_TOKEN + "\n"
        + "Based on this image, what is the most likely hair color of the person doing {activity}? "
        + "Choose from 'black', 'brown', 'blonde', 'red', 'gray', or 'white'."
    )
...
def interpret_hair_color(answer):
    """
    Parses LLaVA's response and classifies it into one of six hair colors.
    """
    lowered = answer[0].lower()
    for color in ["black", "brown", "blonde", "red", "gray", "white"]:
        if color in lowered:
            return color
    return "uncertain"  # Default if hair color is unclear
...
def process_frames_with_llava(
    ...
    sample_size=5,
    seed=42,
    summary_tsv="hair_color_summary.tsv",
    framewise_tsv="hair_color_framewise.tsv"
):
    """
    Iterates over subfolders in frames_root, extracts the activity name from folder name,
    randomly samples up to `sample_size` frames, queries LLaVA to classify hair color,
    and writes to two TSV files:

    1. framewise_tsv (frame-level):
       [video_name, frame_file, hair_color]

    2. summary_tsv (video-level):
       [video_name, black_count, brown_count, blonde_count, red_count, gray_count, white_count, final_label]
    """
    random.seed(seed)
    ...
    with open(summary_tsv, mode="w", newline="", encoding="utf-8") as f_summary, \
        ...
        # Write headers
        summary_writer.writerow(["video_name", "black_count", "brown_count", "blonde_count", "red_count", "gray_count", "white_count", "final"])
        frame_writer.writerow(["video_name", "frame_file", "hair_color"])
        ...
        for video_name in sorted(os.listdir(frames_root)):
            ...
            # Initialize counters for each hair color
            color_counts = {color: 0 for color in ["black", "brown", "blonde", "red", "gray", "white"]}
            ...
            for frame_file in frame_files:
                ...
                # Ask LLaVA for hair color classification
                answer = ask_llava(frame_path, model, tokenizer, image_processor, device, activity)
                hair_color = interpret_hair_color(answer)

                # Write frame-level info
                frame_writer.writerow([video_name, frame_file, hair_color])

                # Tally the results
                if hair_color in color_counts:
                    color_counts[hair_color] += 1

            # Determine final label by majority vote
            final_label = max(color_counts, key=color_counts.get) if max(color_counts.values()) > 0 else "uncertain"

            # Write video-level summary
            summary_writer.writerow([video_name] + list(color_counts.values()) + [final_label])
            print(f"[INFO] {video_name} (Activity: {activity}): {color_counts}, final={final_label}")
...
if __name__ == "__main__":
    ...
    summary_tsv = "workspace/hug/tp/llava_hair_color_summary.tsv"
    framewise_tsv = "workspace/hug/tp/llava_hair_color_framewise.tsv"
